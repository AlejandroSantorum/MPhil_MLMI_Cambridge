...
fname_res=./results/rnn_multiple_layers.txt
fname_plot=./imgs/rnn_multiple_layers.png
...
do
    # Training
    ../tools/steps/step-dnntrain -MODELINI ../new_nn_models/rnn/RNN-${n_layers}L.ReLU.MFCC_E_D_A_Z.ini -GPUID 0 -vvv ../convert/mfc13d/env/environment_E_D_A_Z $pracgmmexp/align-mono-hmm84/align/timit_train.mlf $pracgmmexp/mono/hmm84/MMF $pracgmmexp/mono/hmms.mlist ${store_folder}/RNN-${n_layers}L-MFC_E_D_A_Z-UV${fixed_unfoldVal}/dnntrain
    # Fetching training accuracies: CV set accuracy and train accuracy
    echo "===== Train num_hidden_layers=${n_layers} =====" >> $fname_res
    trainAcc=$(grep 'Train Accuracy' ${store_folder}/RNN-${n_layers}L-MFC_E_D_A_Z-UV${fixed_unfoldVal}/dnntrain/hmm0/LOG | tail -1)
    echo $trainAcc >> $fname_res
    echo "===== Validation num_hidden_layers=${n_layers} =====" >> $fname_res
    cvAcc=$(grep 'Validation Accuracy' ${store_folder}/RNN-${n_layers}L-MFC_E_D_A_Z-UV${fixed_unfoldVal}/dnntrain/hmm0/LOG | tail -1)
    echo $cvAcc >> $fname_res
    # Decoding, i.e., Testing
    ../tools/steps/step-decode -INSWORD -8 -BEAMWIDTH 200 $PWD/${store_folder}/RNN-${n_layers}L-MFC_E_D_A_Z-UV${fixed_unfoldVal}/dnntrain hmm0 ${store_folder}/RNN-${n_layers}L-MFC_E_D_A_Z-UV${fixed_unfoldVal}/decode
    echo "===== Test num_hidden_layers=${n_layers} =====" >> $fname_res
    testAcc=$(grep 'WORD:' ${store_folder}/RNN-${n_layers}L-MFC_E_D_A_Z-UV${fixed_unfoldVal}/decode/test/LOG)
    echo $testAcc >> $fname_res
done
